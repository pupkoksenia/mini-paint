...
export const useFireBasePaints: () => FireBasePaints = () => {
  ...
  const getFeedPaints = () =>
    getDocs(collection(db, 'users')).then((docs) => {
      docs.forEach((doc) => {
        doc.data().paints.forEach((paint: { nameOfPaint: string; date: Timestamp; url: string }) => {
          let isPushed = false

          statePaint.dataPaints?.forEach((p) => {
            if (p.urlOfPaint === paint.url && p.nameOfPaint === paint.nameOfPaint) isPushed = true
          })
  ...
  const filteredItems = computed(() => {
    if (statePaint.filterEmail && statePaint.filterPaint === '')
      return statePaint.dataPaints.filter((p: DataPaint) => p.userName === statePaint.filterEmail)
    else if (statePaint.filterEmail === '' && statePaint.filterPaint)
      return statePaint.dataPaints.filter((p: DataPaint) => p.nameOfPaint === statePaint.filterPaint)
  ...
  const feedPaints = computed(() => {
    const items = filteredItems.value.sort(
      (prev: DataPaint, curr: DataPaint) =>
        new Date(prev.dateInTimestamp.seconds).getTime() - new Date(curr.dateInTimestamp.seconds).getTime()
    )
  ...
  const deleteUserPaint = (NameOfPaint: string, url: string, user: string) => {
    ...
    getDocs(collection(db, 'users'))
      ...
      .then(() => {
        getPaintsOfUser.forEach((paint: { date: Timestamp; nameOfPaint: string; url: string }, index: number) => {
