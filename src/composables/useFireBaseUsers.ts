...
export const useFireBaseUsers: () => FireBaseUsers = () => {
        ...
        getDocs(collection(db, 'users')).then((docs) => {
            docs.forEach((doc) => {
                ...
                stateUsers.dataUsers?.forEach((user) => {
                    if (user.email === doc.data().name && user.role === doc.data().role) isPushed = true
                })
    ...
    const filteredItems = computed(() => {
        if (stateUsers.filterEmailVal && stateUsers.filterRoleVal === '')
            return stateUsers.dataUsers.filter((user: DataUser) => user.email === stateUsers.filterEmailVal)
        else if (stateUsers.filterEmailVal === '' && stateUsers.filterRoleVal)
            return stateUsers.dataUsers.filter((user: DataUser) => user.role === stateUsers.filterRoleVal)
        else if (stateUsers.filterEmailVal && stateUsers.filterRoleVal)
            return stateUsers.dataUsers.filter(
                (user: DataUser) => user.email === stateUsers.filterEmailVal && user.role === stateUsers.filterRoleVal
            )
    ...
    const nextPage = () => {
        if (stateUsers.page !== Math.ceil(filteredItems.value.length / stateUsers.perPage)) {
            stateUsers.page += 1
        }
    }
    const backPage = () => {
